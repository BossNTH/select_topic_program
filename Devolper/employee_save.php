...
$username      = trim($_POST['username'] ?? 'EMP-000');// กำหนด username เริ่มต้น
$password_raw  = $_POST['password'] ?? 'Emp123456'; // กำหนดรหัสผ่านเริ่มต้น
...
try {
  ...
  /* 1) INSERT employees (ยังไม่ใส่ employee_code) 
  ...
  /* 2) สร้างรหัส EMP-xxx จาก employee_id แล้ว UPDATE กลับไปที่ employees */
  ...
  $upd->execute();
  /* 3) INSERT users username จาก empCode */
  $sqlUser = "INSERT INTO users (username, password, role) VALUES (?, ?, ?)";
  $stmtUser = $conn->prepare($sqlUser);
  $stmtUser->bind_param("sss", $empCode, $password, $role);
  $stmtUser->execute();
